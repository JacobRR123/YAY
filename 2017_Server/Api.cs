...
namespace server
{
	internal class APIServer
	{
			...
			try
			{
					...
					for (; ; )
					{
						...
						if (Url == "platformlogin/v1/profiles")
						{
							...
							if (new WebClient().DownloadString("https://raw.githubusercontent.com/JacobRR123/NetRec/master/Update/banned.txt").Contains(File.ReadAllText("SaveData\\Profile\\userid.txt")))
							{
								...
								Console.WriteLine("You are banned. Using this version of NetRec V2 will not work, please download NetRec V2 0.4.2 or prior.");
								Console.ForegroundColor = ConsoleColor.Red;
				...
				else if (start.Program.version == "2018")
				{
					for (; ; )
					{
						...
						if (Url == "rooms/v1/featuredRoomGroup")
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/JacobRR123/NetRec/master/Update/dormslideshow.txt");
						}
						if (Url.StartsWith("rooms/v1/hot"))
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/JacobRR123/NetRec/master/Update/hotrooms.txt");
						}
						...
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/JacobRR123/NetRec/master/Update/rcslideshow.txt");
						}
						Console.WriteLine("API Response: " + s);
						bytes = Encoding.UTF8.GetBytes(s);
						response.ContentLength64 = (long)bytes.Length;
		...
		public static string Banned = "{\"ReportCategory\":1,\"Duration\":100,\"GameSessionId\":200,\"Message\":\"You have been banned. If You Think This ban Is false ban DM me to appeal Jake123#8025.\"}";
